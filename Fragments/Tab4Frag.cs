...
namespace BitChute.Fragments
{
    public class Tab4Frag : Android.Support.V4.App.Fragment
    {
        ...
        {
            if(_hideverticalnavbaronrb.Checked){AppSettings.HideVerticalNavBar=true;}
            else{AppSettings.HideVerticalNavBar = false;}
        ...
        public void CustomLoadUrl(string url){ Wv.LoadUrl(url);}
        ...
        private void OnFanModeRbCheckChanged(object sender, EventArgs e)
        {
            if (!AppNowCheckingBoxes)
            {
                if (_fmonrb.Checked) { MainActivity.TabDetailChanger(3, AppSettings.Tab4OverridePreference); }
                else{MainActivity.TabDetailChanger(3, "MyChannel");}
            }
        }
        ...
        private void OnSettingsTabOverrideSpinnerSelectionChanged(object sender, EventArgs e)
        {
            if (!AppNowCheckingBoxes)
            {
                ...
                if (_fmonrb.Checked){MainActivity.TabDetailChanger(4, AppSettings.Tab5OverridePreference); }
                else{}
        ...
        public void Pop2Root()
        {
            if (_wvRl)
            {
                try { Wv.Reload(); _wvRl = false; }
                catch { }
            }
            else { Wv.LoadUrl(RootUrl); }
        }
        private static bool _wvRling = false;
        /// <summary>
